function run_GLEAM_mouse()

load('mouse_GLEAM.mat')
...
% （dを与える　→　計測met推定）　→　計測met固定　→　（dを与える　→　未計測met推定）　→　d固定して最適解計算し直し
%　　↓
% （dを与える　→　計測met推定　→　計測met固定　→　未計測met推定）　→　収束即最適解

% run_GLEAM_mouse　→　[ create_obj_func　→　GLEAM_opt　→　{ gaqp　→　( opt_measured　→　opt_unmeasured )　→　opt_get_sol　→　( opt_measured　→　opt_unmeasured ) } ]　
% →　cal_redchi2　→　[ perturb_m　→　coeff_gen_m　→　GLEAM_opt_direcFixed　→　{ opt_get_sol　→　( opt_measured　→　opt_unmeasured ) }　→　checkbootstrap ]　
% →　信頼区間計算