function [meg, c] = cal_meg(x,x_invivo,Ac,Af,T,CC,ratio,lb,ub,rxn,rev,mets,d,h,RT)
% calculate MEG at ECM
revrxn = intersect(rev,rxn);
tp=14;
rG_lim_rev = zeros(size(Ac,1),1);
rG_lim_rev = rG_lim_rev - Af*T * x_invivo(size(Ac,2)*tp+1:size(Ac,2)*tp+size(Af,2));
...
rG_lim_irrev = rG_lim_irrev - Af*T.*d * x_invivo(size(Ac,2)*tp+1:size(Ac,2)*tp+size(Af,2));
