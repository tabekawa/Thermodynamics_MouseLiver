function [X,F,RSS,Ps,resid,result,err] = opt_gqp_t_m2(H,H_unm,f_unm,Ac,Af,C,ratio,lb,ub,sq,inv_nsigma,smpl,sigma,lambda,T,tp,d,f_exp,sq_exp)
    ...
    inv_nsigma = zeros(size(inv_nsigma,1),1);
    ...
    if strcmp(RESULT.status, 'INFEASIBLE') || strcmp(RESULT.status, 'INF_OR_UNBD') || strcmp(RESULT.status, 'NUMERIC')
        ...
        X = RESULT.x;
        F = RESULT.objval;
        err = X(1:size(f_exp,1))' * (H_pre./2) * X(1:size(f_exp,1)) + f_exp' * X(1:size(f_exp,1)) + sq_exp;
